...
{
    if (closesocket(tcp_conn->socket) == SOCKET_ERROR)
    {
        net_log_error(WSAGetLastError(), "Failed to close TCP socket: ");
        return -1;
    }
    return 0;
}
...
network_create_udp_socket(UDP_Connection* out_conn, int async)
{
    ...
    {
        net_log_error(WSAGetLastError(), "Failed to create udp socket: ");
        return -1;
    }
...
network_receive_udp_packets(UDP_Connection* udp_conn, UDP_Packet* out_packet)
{
    ...
    int status = recvfrom(udp_conn->socket, buffer, PACKET_SIZE, 0, (struct sockaddr*)&sender_info, &info_size_bytes);
    if (status == 0)
    {
...
network_create_tcp_socket(TCP_Connection* out_conn, int async)
{
    ...
    {
        net_log_error(WSAGetLastError(), "Failed to create tcp socket: ");
        return -1;
    }
    ...
    net_log_info("Created net tcp socket (%s)\n", (async) ? "async" : "blocking");
    return 0;
}
...
network_create_tcp_bound_socket(TCP_Connection* out_conn, unsigned short port, int async)
{
    ...
    net_log_info("Created net tcp socket (%s) bound to %d", (async) ? "async" : "blocking", port);
    ...
    if (bind(connection_socket, (struct sockaddr*)&server_info, sizeof(server_info)) == SOCKET_ERROR)
    {
        net_log_error(WSAGetLastError(), "Failed to bind socket to port %d: ", port);
        closesocket(connection_socket);
        return -1;
    }

    net_log_info("Bound net tcp socket (%s) to %d", (async) ? "async" : "blocking", port);
...
{
    if (connect(conn->socket, (const struct sockaddr*)addr, sizeof(struct sockaddr_in)) != 0) 
    {
        int err = WSAGetLastError();
        Net_Status result = NETWORK_OK;
        switch(err)
        {
            case WSANOTINITIALISED: result = NETWORK_UNINITIALIZED;
            case WSAENETDOWN:       result = NETWORK_UNREACHABLE;
            case WSAEADDRINUSE:     result = NETWORK_ADDR_IN_USE;
            case WSAEAFNOSUPPORT:
            case WSAEFAULT:
            case WSAEADDRNOTAVAIL:  result = NETWORK_INVALID_ADDRESS;
            case WSAECONNREFUSED:   result = NETWORK_CONN_REFUSED;
            case WSAEINVAL:         result = NETWORK_ALREADY_LISTENING;
            case WSAEISCONN:        result = NETWORK_ALREADY_CONNECTED;
            case WSAEHOSTUNREACH:   result = NETWORK_HOST_UNREACHEABLE;
            case WSAENETUNREACH:    result = NETWORK_UNREACHABLE;
            case WSAETIMEDOUT:      result = NETWORK_CONN_TIMEOUT;
            case WSAEWOULDBLOCK:    result = NETWORK_PACKET_NONE;
            default: {
                net_log_error(WSAGetLastError(), "Failed to connect to server: ");
...
network_send_tcp_packet(TCP_Connection* tcp_conn, const char* data, int length)
{
    ...
    if (bytes_sent == SOCKET_ERROR)
    {
        ...
        switch (err)
        {
            case WSANOTINITIALISED: return NETWORK_UNINITIALIZED;
            case WSAEWOULDBLOCK:   return NETWORK_PACKET_NONE;
            case WSAECONNABORTED:
            case WSAESHUTDOWN:     return NETWORK_FORCED_SHUTDOWN;
            case WSAECONNRESET:    return NETWORK_PORT_UNREACHEABLE;
...
{
    if (listen(tcp_conn->socket, count) != 0) {
        int err = WSAGetLastError();
        ...
        switch(err)
        {
            ...
            case WSAEADDRINUSE: result = NETWORK_ADDR_IN_USE;
            case WSAENETDOWN:   result = NETWORK_UNREACHABLE;
            case WSAEINVAL:     result = NETWORK_SOCKET_NOT_BOUND;
            case WSAEISCONN:    result = NETWORK_ALREADY_CONNECTED;
            default: {
                net_log_error(WSAGetLastError(), "Failed to listen for connection: ");
                return NETWORK_ERROR;
            } break;
        }
...
// struct sockaddr_in* connected, SOCKET* new_socket
Net_Status
network_accept(TCP_Connection* tcp_conn, TCP_Connection* new_connection)
{
    int len = sizeof(struct sockaddr_in);
    new_connection->socket = accept(tcp_conn->socket, (struct sockaddr*)&new_connection->addr, &len);
    if (new_connection->socket == INVALID_SOCKET) {
        int err = WSAGetLastError();
        Net_Status result = NETWORK_PACKET_NONE;
        switch(err)
        {
            case WSANOTINITIALISED: result = NETWORK_UNINITIALIZED;
            case WSAECONNRESET:     result = NETWORK_PORT_UNREACHEABLE;
            case WSAEINVAL:         result = NETWORK_NOT_LISTENING;
            case WSAENETDOWN:       result = NETWORK_UNREACHABLE;
            case WSAEWOULDBLOCK:    result = NETWORK_OK;
            default: result = NETWORK_ERROR;
        }
        if (result != NETWORK_OK)
            net_log_error(WSAGetLastError(), "Failed to accept connection: ");
        return result;
    }
    return NETWORK_OK;
}